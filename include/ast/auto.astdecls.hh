...
struct returnStmt : AST {Node <expression> rtrn_expr;returnStmt();returnStmt(Node <expression> _rtrn_expr);virtual std::vector <std::string> contents () const ; };
struct var : AST {Node <terminal> name;Node <addExpr> size;var();var(Node <terminal> _name, Node <addExpr> _size);virtual std::vector <std::string> contents () const ; };
struct expression : AST {Node <expression> lhs;Node <relop> op;Node <addExpr> rhs;expression();expression(Node <expression> _lhs, Node <relop> _op, Node <addExpr> _rhs);virtual std::vector <std::string> contents () const ; };
struct relop : AST {Node <terminal> sym;relop();relop(Node <terminal> _sym);virtual std::vector <std::string> contents () const ; };
struct addExpr : AST {Node <addExpr> lhs;Node <addop> op;Node <term> rhs;addExpr();addExpr(Node <addExpr> _lhs, Node <addop> _op, Node <term> _rhs);virtual std::vector <std::string> contents () const ; };
struct addop : AST {Node <terminal> sym;addop();addop(Node <terminal> _sym);virtual std::vector <std::string> contents () const ; };
...
struct mulop : AST {Node <terminal> sym;mulop();mulop(Node <terminal> _sym);virtual std::vector <std::string> contents () const ; };
struct factor : AST {AST_node rvalue;factor();factor(AST_node _rvalue);virtual std::vector <std::string> contents () const ; };
struct funcCallExpr : AST {Node <terminal> name;Node <argList> arg_list;funcCallExpr();funcCallExpr(Node <terminal> _name, Node <argList> _arg_list);virtual std::vector <std::string> contents () const ; };
