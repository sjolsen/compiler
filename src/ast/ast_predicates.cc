			...
			if (tokens [0].type == token_type::symbol)
				...
				case symbol::rparen:
					...
					if (open_tokens.top ()->op != symbol::lparen)
						throw syntax_error ("Unmatched '" + to_string (open_tokens.top ()->op) + "'",
						                    open_tokens.top ()->pos);
					open_tokens.pop ();
					break;
				case symbol::rbrace:
...
Node <varDecl> varDecl_p (token_range& tokens)
{
	...
	if (lbracket_node)
	{
		...
		if (!num)
			throw syntax_error ("Expected numeric constant",
			                    begin (working_set)->pos);
...
Node <funBody> funBody_p (token_range& tokens)
{
	...
	if (!rbrace_node)
		throw syntax_error ("Expected '}', declaration, or statement",
		                    begin (working_set)->pos);
...
Node <compoundStmt> compoundStmt_p (token_range& tokens)
{
	...
	AST_node rbrace_node = get_token (working_set, symbol::rbrace);
	if (!rbrace_node)
		throw syntax_error ("Expected '}' or statement",
		                    begin (working_set)->pos);
...
Node <condStmt> condStmt_p (token_range& tokens)
{
	...
	if (!if_statement)
		throw syntax_error ("Expected statement",
		                    begin (working_set)->pos);
