declList::declList (std::vector <Node <decl>>&& _decls): decls(move (_decls)) {type = AST_type::declList;link (this, decls);}
...
decl::decl (Node <basic_decl>&& _sub_decl): sub_decl(move (_sub_decl)) {type = AST_type::decl;link (this, sub_decl);}
decl::decl () {type = AST_type::decl;}
vector <string> decl::contents () const {return collect (valid (sub_decl) ? lines (sub_decl) : vector <string> {} ); }
typeSpecifier::typeSpecifier (Node <terminal>&& _kwd_node): kwd_node(move (_kwd_node)) {type = AST_type::typeSpecifier;link (this, kwd_node);}
...
formalDeclList::formalDeclList (std::vector <Node <formalDecl>>&& _decls): decls(move (_decls)) {type = AST_type::formalDeclList;link (this, decls);}
...
funBody::funBody (Node <localDeclList>&& _decl_list, Node <statementList>&& _stmt_list): decl_list(move (_decl_list)) , stmt_list(move (_stmt_list)) {type = AST_type::funBody;link (this, decl_list, stmt_list);}
...
localDeclList::localDeclList (std::vector <Node <varDecl>>&& _decls): decls(move (_decls)) {type = AST_type::localDeclList;link (this, decls);}
...
statementList::statementList (std::vector <Node <statement>>&& _stmts): stmts(move (_stmts)) {type = AST_type::statementList;link (this, stmts);}
...
statement::statement (AST_node&& _sub_stmt): sub_stmt(move (_sub_stmt)) {type = AST_type::statement;link (this, sub_stmt);}
...
compoundStmt::compoundStmt (Node <statementList>&& _stmt_list): stmt_list(move (_stmt_list)) {type = AST_type::compoundStmt;link (this, stmt_list);}
...
assignStmt::assignStmt (Node <var>&& _lvalue, Node <expression>&& _rvalue): lvalue(move (_lvalue)) , rvalue(move (_rvalue)) {type = AST_type::assignStmt;link (this, lvalue, rvalue);}
...
condStmt::condStmt (Node <expression>&& _cond_expr, Node <statement>&& _then_stmt, Node <statement>&& _else_stmt): cond_expr(move (_cond_expr)) , then_stmt(move (_then_stmt)) , else_stmt(move (_else_stmt)) {type = AST_type::condStmt;link (this, cond_expr, then_stmt, else_stmt);}
...
expression::expression (Node <expression>&& _lhs, Node <relop>&& _op, Node <addExpr>&& _rhs): lhs(move (_lhs)) , op(move (_op)) , rhs(move (_rhs)) {type = AST_type::expression;link (this, lhs, op, rhs);}
...
relop::relop (Node <terminal>&& _sym): sym(move (_sym)) {type = AST_type::relop;link (this, sym);}
relop::relop () {type = AST_type::relop;}
vector <string> relop::contents () const {return collect (valid (sym) ? lines (sym) : vector <string> {} ); }
addExpr::addExpr (Node <addExpr>&& _lhs, Node <addop>&& _op, Node <term>&& _rhs): lhs(move (_lhs)) , op(move (_op)) , rhs(move (_rhs)) {type = AST_type::addExpr;link (this, lhs, op, rhs);}
...
addop::addop (Node <terminal>&& _sym): sym(move (_sym)) {type = AST_type::addop;link (this, sym);}
addop::addop () {type = AST_type::addop;}
vector <string> addop::contents () const {return collect (valid (sym) ? lines (sym) : vector <string> {} ); }
term::term (Node <term>&& _lhs, Node <mulop>&& _op, Node <factor>&& _rhs): lhs(move (_lhs)) , op(move (_op)) , rhs(move (_rhs)) {type = AST_type::term;link (this, lhs, op, rhs);}
...
mulop::mulop (Node <terminal>&& _sym): sym(move (_sym)) {type = AST_type::mulop;link (this, sym);}
mulop::mulop () {type = AST_type::mulop;}
vector <string> mulop::contents () const {return collect (valid (sym) ? lines (sym) : vector <string> {} ); }
factor::factor (AST_node&& _rvalue): rvalue(move (_rvalue)) {type = AST_type::factor;link (this, rvalue);}
...
funcCallExpr::funcCallExpr (Node <terminal>&& _name, Node <argList>&& _arg_list): name(move (_name)) , arg_list(move (_arg_list)) {type = AST_type::funcCallExpr;link (this, name, arg_list);}
...
argList::argList (std::vector <Node <expression>>&& _args): args(move (_args)) {type = AST_type::argList;link (this, args);}
