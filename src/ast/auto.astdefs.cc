declList::declList (std::vector <Node <decl>>&& _decls): decls(move (_decls)) {type = AST_type::declList;}
...
decl::decl (Node <basic_decl>&& _sub_decl): sub_decl(move (_sub_decl)) {type = AST_type::decl;}
decl::decl () {type = AST_type::decl;}
vector <string> decl::contents () const {return collect (valid (sub_decl) ? lines (sub_decl) : vector <string> {} ); }
typeSpecifier::typeSpecifier (Node <terminal>&& _kwd_node): kwd_node(move (_kwd_node)) {type = AST_type::typeSpecifier;}
...
formalDeclList::formalDeclList (std::vector <Node <formalDecl>>&& _decls): decls(move (_decls)) {type = AST_type::formalDeclList;}
...
funBody::funBody (Node <localDeclList>&& _decl_list, Node <statementList>&& _stmt_list): decl_list(move (_decl_list)) , stmt_list(move (_stmt_list)) {type = AST_type::funBody;}
...
localDeclList::localDeclList (std::vector <Node <varDecl>>&& _decls): decls(move (_decls)) {type = AST_type::localDeclList;}
...
statementList::statementList (std::vector <Node <statement>>&& _stmts): stmts(move (_stmts)) {type = AST_type::statementList;}
...
statement::statement (AST_node&& _sub_stmt): sub_stmt(move (_sub_stmt)) {type = AST_type::statement;}
...
compoundStmt::compoundStmt (Node <statementList>&& _stmt_list): stmt_list(move (_stmt_list)) {type = AST_type::compoundStmt;}
...
assignStmt::assignStmt (Node <var>&& _lvalue, Node <expression>&& _rvalue): lvalue(move (_lvalue)) , rvalue(move (_rvalue)) {type = AST_type::assignStmt;}
...
condStmt::condStmt (Node <expression>&& _cond_expr, Node <statement>&& _then_stmt, Node <statement>&& _else_stmt): cond_expr(move (_cond_expr)) , then_stmt(move (_then_stmt)) , else_stmt(move (_else_stmt)) {type = AST_type::condStmt;}
...
loopStmt::loopStmt (Node <expression>&& _cond_expr, Node <statement>&& _then_stmt): cond_expr(move (_cond_expr)) , then_stmt(move (_then_stmt)) {type = AST_type::loopStmt;}
...
returnStmt::returnStmt (Node <expression>&& _rtrn_expr): rtrn_expr(move (_rtrn_expr)) {type = AST_type::returnStmt;}
...
var::var (Node <terminal>&& _name, Node <addExpr>&& _size): name(move (_name)) , size(move (_size)) {type = AST_type::var;}
...
expression::expression (Node <expression>&& _lhs, Node <relop>&& _op, Node <addExpr>&& _rhs): lhs(move (_lhs)) , op(move (_op)) , rhs(move (_rhs)) {type = AST_type::expression;}
...
relop::relop (Node <terminal>&& _sym): sym(move (_sym)) {type = AST_type::relop;}
relop::relop () {type = AST_type::relop;}
vector <string> relop::contents () const {return collect (valid (sym) ? lines (sym) : vector <string> {} ); }
addExpr::addExpr (Node <addExpr>&& _lhs, Node <addop>&& _op, Node <term>&& _rhs): lhs(move (_lhs)) , op(move (_op)) , rhs(move (_rhs)) {type = AST_type::addExpr;}
...
addop::addop (Node <terminal>&& _sym): sym(move (_sym)) {type = AST_type::addop;}
addop::addop () {type = AST_type::addop;}
vector <string> addop::contents () const {return collect (valid (sym) ? lines (sym) : vector <string> {} ); }
term::term (Node <term>&& _lhs, Node <mulop>&& _op, Node <factor>&& _rhs): lhs(move (_lhs)) , op(move (_op)) , rhs(move (_rhs)) {type = AST_type::term;}
...
mulop::mulop (Node <terminal>&& _sym): sym(move (_sym)) {type = AST_type::mulop;}
mulop::mulop () {type = AST_type::mulop;}
vector <string> mulop::contents () const {return collect (valid (sym) ? lines (sym) : vector <string> {} ); }
factor::factor (AST_node&& _rvalue): rvalue(move (_rvalue)) {type = AST_type::factor;}
...
funcCallExpr::funcCallExpr (Node <terminal>&& _name, Node <argList>&& _arg_list): name(move (_name)) , arg_list(move (_arg_list)) {type = AST_type::funcCallExpr;}
...
argList::argList (std::vector <Node <expression>>&& _args): args(move (_args)) {type = AST_type::argList;}
